...
static struct list sleep_thread_list;   /* list of sleeping thread */

/* compare the wake up tick of the newly inserting thread and  the 
   thread in the list. */
static bool
...
timer_sleep (int64_t ticks)
{
  ...
  /* struct thread의 wake_up_tick 값 업데이트 */
  cur_thread = thread_current();
  cur_thread -> wake_up_tick = start + ticks;

  /* sleep_thread_list에 추가 */
  list_insert_ordered (&sleep_thread_list, &cur_thread->elem, 
                       cmp_tick, NULL);
  
  /* sleeping thread를 block(waiting) 상태로 만들기 */
  thread_block ();
