               ...
               thread_func *function, void *aux) 
{
  ...
  t->info->tid = t->tid;
  sema_init (&t->info->exit_sema, 0);
  t->info->thread = t;
  list_push_front (&thread_current ()->child_list, &t->info->elem);
  hash_init (&t->upage_table, page_hash, page_less, NULL);
#endif
  if (thread_current () != NULL && thread_current ()->cur_dir != NULL)
...
/* compare the priority of the newly inserting thread and the 
   thread in the list. */
bool
...
thread_set_priority (int new_priority) 
{
  ...
  /* donation 받은 priority가 더 큰 경우 */
  else if (cur->priority > new_priority)
    cur->origin_priority = new_priority;
  /* donation 받은 priority가 더 작은 경우 */
  else
  {
    cur->origin_priority = -1;
    cur->priority = new_priority;
  }
...
/* 같은 이름을 가진 파일이 현재 실행중인지 확인한다. */
bool
check_file_executing (char *filename)
{
  /* all_list 의 head 로 이동 */
  struct list_elem *e;
  for (e = list_begin (&all_list); e != list_end (&all_list);
       e = list_next (e))
  {
